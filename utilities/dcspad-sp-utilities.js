 ...
 * Point browser to: https://bmo.sharepoint.com/teams/FCUCommunicationsCreativeServices/SitePages/PNPJs3-test.aspx
 * Then use window.SPUtils.functionName(params...) in browser console
 * 
 * @example
 ...
 *   { dryRun: false, adminEmail: "admin@bmo.com", umbrellaGroup: "Marketing Team" }
 * );
 * 
 *  * =============================
...
let siteContextRelativePath = null;
let contextInitialized = false;
const standardAdmin = "joseph.zapert@bmo.com";

window.SPUtils = (() => {
    const SPTenantBase = "https://bmo.sharepoint.com/";
    ...
    // ---------------------------------------------------------------
    const USAGE = [
      { name: "setupContext", sig: "(siteSubUrl)", what: "Call FIRST. Points PnPjs at a site.",
        example: `SPUtils.setupContext("teams/MyTeamSite")` },
     ...
     * 
     * @param {string} siteSubUrl - Relative URL path (e.g., "teams/Marketing" or "sites/MyProject")
     *                               Will be appended to SPTenantBase (https://bmo.sharepoint.com/)
    ...
    const setupContext = (siteSubUrl) => {
      try {
        pnp2.sp.setup({ sp: { baseUrl: SPTenantBase + siteSubUrl } });
        writeStatus("setupContext", `Context set to: ${SPTenantBase + siteSubUrl}`);
        siteContextRelativePath = siteSubUrl;
...
do {
  const url = nextLink ||
    `${SPTenantBase}${siteContextRelativePath}/_api/web/lists/getByTitle('${listTitle}')/items?$top=200`;
